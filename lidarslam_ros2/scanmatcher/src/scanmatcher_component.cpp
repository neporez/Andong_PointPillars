  ...
  broadcaster_(this)
{
  ...
  //NDT 중첩 Pointcloud 생성 플래그
  declare_parameter("ndt_accumulate_stack", 5);
  get_parameter("ndt_accumulate_stack", ndt_accumulate_stack_);

  declare_parameter("ndt_accumulate_cloud_topic_name", "/ndt_accumulate_cloud");
  get_parameter("ndt_accumulate_cloud_topic_name", ndt_accumulate_topic_name_);
...
void ScanMatcherComponent::initializePubSub()
{
  ...
  //ndt 중첩 Pointcloud Publisher

  ndt_accumulate_cloud_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(ndt_accumulate_topic_name_, rclcpp::QoS(10));
...
void ScanMatcherComponent::initializeMap(const pcl::PointCloud <pcl::PointXYZI>::Ptr & tmp_ptr, const std_msgs::msg::Header & header)
{
  ...
  if(ndt_accumulate_stack_ > 0) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr transformed_tmp_ptr(new pcl::PointCloud<pcl::PointXYZI>());
    pcl::transformPointCloud(*tmp_ptr, *transformed_tmp_ptr, sim_trans);

    sensor_msgs::msg::PointCloud2 current_cloud;
    pcl::toROSMsg(*transformed_tmp_ptr, current_cloud);
    current_cloud.header.frame_id = robot_frame_id_;
    current_cloud.header.stamp = rclcpp::Clock().now();

    ndt_accumulate_cloud_queue_.push(current_cloud);
  }
  ...
  const rclcpp::Time stamp)
{
  ...
  if(ndt_accumulate_stack_ > 0) {
    ...
    pcl::transformPointCloud(*cloud_ptr, transformed_cloud, final_transformation);

    sensor_msgs::msg::PointCloud2 transformed_cloud_msg;
    pcl::toROSMsg(transformed_cloud, transformed_cloud_msg);
    transformed_cloud_msg.header.frame_id = robot_frame_id_;
    transformed_cloud_msg.header.stamp = rclcpp::Clock().now();

    transformed_cloud_msg.width = transformed_cloud.size();
    transformed_cloud_msg.height = 1;
    transformed_cloud_msg.is_dense = true;


    // 큐 크기가 최대 크기 초과 시 가장 오래된 항목 제거
    if (ndt_accumulate_cloud_queue_.size() >= ndt_accumulate_stack_) {
      ndt_accumulate_cloud_queue_.pop();
    }
    ndt_accumulate_cloud_queue_.push(transformed_cloud_msg);
  }
  ...
  // 실행 시간 계산 및 출력 (마이크로초 단위)
  //auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
  //RCLCPP_INFO(get_logger(), "publishAccumulatedPointCloud 실행 시간: %ld µs", duration);
...
void ScanMatcherComponent::publishAccumulatedPointCloud()
{
  if (ndt_accumulate_stack_ <= 0) return;

  pcl::PointCloud<pcl::PointXYZI> accumulated_cloud;

  Eigen::Vector3f current_position(
      corrent_pose_stamped_.pose.position.x,
      corrent_pose_stamped_.pose.position.y,
      corrent_pose_stamped_.pose.position.z);
  Eigen::Quaternionf current_orientation(
      corrent_pose_stamped_.pose.orientation.w,
      corrent_pose_stamped_.pose.orientation.x,
      corrent_pose_stamped_.pose.orientation.y,
      corrent_pose_stamped_.pose.orientation.z);

  std::queue<sensor_msgs::msg::PointCloud2> temp_queue = ndt_accumulate_cloud_queue_;

  Eigen::Quaternionf inverse_orientation = current_orientation.inverse();

  while (!temp_queue.empty()) {
    sensor_msgs::msg::PointCloud2 cloud_msg = temp_queue.front();
    temp_queue.pop();
    ...
    for (auto & point : cloud) {
      ...
      point_position = inverse_orientation * (point_position - current_position);
      point.x = point_position.x();
      point.y = point_position.y();
      point.z = point_position.z();
      transformed_cloud.push_back(point);  // Add transformed point
    }


    // 누적된 클라우드에 변환된 포인트들을 추가
    for (const auto & point : transformed_cloud) {
      accumulated_cloud.push_back(point);
    }
  }

  sensor_msgs::msg::PointCloud2 accumulated_cloud_msg;
  pcl::toROSMsg(accumulated_cloud, accumulated_cloud_msg);

  accumulated_cloud_msg.header.frame_id = robot_frame_id_;
  accumulated_cloud_msg.header.stamp = rclcpp::Clock().now();

  accumulated_cloud_msg.width = accumulated_cloud.size();
  accumulated_cloud_msg.height = 1;
  accumulated_cloud_msg.is_dense = true;

  ndt_accumulate_cloud_pub_->publish(accumulated_cloud_msg);
}
