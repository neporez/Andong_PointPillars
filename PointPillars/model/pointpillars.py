...
#         if stride == 1:
#             self.conv2 = nn.Conv2d(in_channels, out_channels, 1, stride=1, bias=False)
#         else:
#             self.conv2 = None
...
#                 y2 = self.conv2(x)
#                 y3 = x
#                 return F.relu(self.bn1(y1 + y2 + y3))
#             else:
#                 return F.relu(self.bn1(y1))
#         else:
...
#         self.original_block = copy.deepcopy(self)
...
#     def _fuse(self):
#         kernel = self.conv1.weight.clone()
#         if self.stride == 1:
#             kernel1x1 = F.pad(self.conv2.weight, [1, 1, 1, 1])
#             kernel = kernel + kernel1x1
#             identity = torch.eye(self.out_channels, device=kernel.device).reshape(self.out_channels, self.out_channels, 1, 1)
...
#         self.__dict__.update(self.original_block.__dict__)
#         self.original_block = None
...
#         for i in range(len(layer_strides)):
#             blocks = []
#             blocks.append(RepVGGBlock(in_channel, out_channels[i], stride=layer_strides[i]))
#             for _ in range(1, layer_nums[i]):
#                 blocks.append(RepVGGBlock(out_channels[i], out_channels[i]))
#             in_channel = out_channels[i]
#             self.multi_blocks.append(nn.Sequential(*blocks))
...
#     def forward(self, x):
#         outs = []
#         for blocks in self.multi_blocks:
#             x = blocks(x)
#             outs.append(x)
#         return outs
